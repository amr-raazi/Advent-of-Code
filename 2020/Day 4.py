...
# define validators and dict converters
def field_validator(passport: dict):
...
def value_validator(passport: dict):
    if not 1920 <= int(passport.get("byr")) <= 2002: return False
    if not 2010 <= int(passport.get("iyr")) <= 2020: return False
    if not 2020 <= int(passport.get("eyr")) <= 2030: return False
    height = passport.get("hgt")
    if "cm" in height:
        if not 150 <= int(height.replace("cm", "")) <= 193: return False
    elif "in" in height:
        if not 59 <= int(height.replace("in", "")) <= 76: return False
    else: return False
    hcl = passport.get("hcl")
    if "#" not in hcl: return False
    try: int(hcl.replace("#", ""), 16)
    except ValueError: return False
    if passport.get("ecl") not in "amb blu brn gry grn hzl oth": return False
    if len(passport.get("pid")) != 9: return False
    return True
...
for x in inp:
    x = convert_to_dict(x)
    if field_validator(x):
        c += 1
        if value_validator(x):
            z += 1
print(c)
print(z)
